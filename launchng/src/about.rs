...
pub fn about_window(wnd: &gui::WindowMain) -> Result<(), ()> {
    ...
    let version_message = match (is_server(), OsVersion::current()) {
        ...
        (true, version) if version >= OsVersion::new(6, 3, 0, 9600) => "Windows Server 2012 R2".to_string(),
        ...
        (false, version) if version >= OsVersion::new(6, 3, 0, 9600) => format!("Windows 8.1 Build {:?}", OsVersion::current().build),
